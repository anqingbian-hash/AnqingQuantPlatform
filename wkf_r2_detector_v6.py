...
# -*- coding: utf-8 -*-
"""
WKF R2 Line Detector V6 - Smart Candidate Selection
智能候选选择版R2（阻力线）识别器 - 基于颜色和位置的加权评分
"""
...
class WKFR2DetectorV6:
    """R2 (Resistance Line) Detector V6 - Smart Candidate Selection"""
    
    def __init__(self):
        """Initialize detector with smart candidate selection"""
        self.regions = {
        ...
        self.selection_weights = {
            'length_weight': 0.3,        # 长度权重（不要太长）
            'brightness_weight': 0.4,     # 亮度权重（白色/浅灰色优先）
            'position_weight': 0.2,      # 位置权重（上半区域优先）
            'color_weight': 0.1           # 颜色权重（白色优先）
        }
    
    def detect_r2_line(self, image_path: str) -> Dict:
        """Detect R2 (resistance line) using smart candidate selection"""
        print(f"\n[R2 Detection V6] Analyzing: {os.path.basename(image_path)}")
        ...
        # [3] Analyze all horizontal lines with smart scoring
        print("  [3] Analyzing horizontal lines with smart scoring...")
        ...
        # [4] Select R2 candidate (highest smart score)
        print("  [4] Selecting R2 candidate (highest smart score)...")
        ...
        if horizontal_lines:
            ...
            print(f"    Result: R2 detected (highest smart score)")
    ...
    def _calculate_smart_score(self, length: float, brightness: float, y: int, 
                               chart_height: int, color_category: str) -> float:
        """Calculate smart score for R2 candidate"""
        ...
        # [2] Brightness score (white/light gray is better)
        ...
        # [4] Color score (white/light gray is better)
        if color_category == 'white':
            color_score = 1.0
        elif color_category == 'light_gray':
            color_score = 0.8
    ...
    def _classify_color(self, b: float, g: float, r: float) -> str:
        ...
        # White (very bright, R≈G≈B)
        if brightness >= 220 and max(abs(b - g), abs(g - r), abs(b - r)) < 30:
            return 'white'
        
        # Light gray (bright, but not white)
        elif brightness >= 180:
            return 'light_gray'
...
def test_r2_detector_v6():
    """Test R2 detector V6 on 5 images"""
    ...
    detector = WKFR2DetectorV6()
    
    print("=" * 80)
    print("WKF R2 Detector V6 - Smart Candidate Selection")
    print("=" * 80)
    print("Smart Selection Weights:")
    ...
    # Save results
    output_file = "/root/.openclaw/workspace/r2_detection_v6_results.json"
    try:
