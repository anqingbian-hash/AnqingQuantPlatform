...
# -*- coding: utf-8 -*-
"""
WKF R2 Line Detector V8 - Grid Line Avoidance
避免网格线版R2识别器 - R2线是中短长度的水平线（500-1500px），不是超长网格线（>2000px）
"""
...
class WKFR2DetectorV8:
    """R2 (Resistance Line) Detector V8 - Grid Line Avoidance"""
    
    def __init__(self):
        """Initialize detector with grid line avoidance"""
        self.regions = {
        ...
        self.hough_params = {
            'rho': 1,
            'theta': np.pi/180,
            'threshold': 10,  # 降低阈值，检测更多短线
            'minLineLength': 100,
            'maxLineGap': 100
        }
        
        # Grid line avoidance weights
        self.selection_weights = {
            'length_weight': 0.30,       # 长度权重（中等长度优先）
            'brightness_weight': 0.30,    # 亮度权重（黑色优先）
            'position_weight': 0.20,      # 位置权重（上半区域优先）
            'color_weight': 0.20         # 颜色权重（黑色优先）
        }
        
        # R2线长度范围（基于实际测量）
        self.r2_length_range = (300, 1500)  # 300-1500像素
    
    def detect_r2_line(self, image_path: str) -> Dict:
        """Detect R2 (resistance line) using grid line avoidance"""
        print(f"\n[R2 Detection V8] Analyzing: {os.path.basename(image_path)}")
        ...
        # [3] Analyze all horizontal lines with grid line avoidance
        print("  [3] Analyzing horizontal lines with grid line avoidance...")
        ...
        for line in lines:
            ...
            # Calculate grid line avoidance score
            score = self._calculate_grid_line_avoidance_score(
        ...
        # [4] Select R2 candidate (highest grid line avoidance score)
        print("  [4] Selecting R2 candidate (highest grid line avoidance score)...")
        
        if horizontal_lines:
            # Sort by grid line avoidance score (highest first)
            ...
            print(f"    Result: R2 detected (highest grid line avoidance score)")
    ...
    def _calculate_grid_line_avoidance_score(self, length: float, brightness: float, y: int, 
                                             chart_height: int, color_category: str) -> float:
        """Calculate grid line avoidance score for R2 candidate"""
        
        score = 0.0
        
        # [1] Length score (grid line avoidance)
        # R2线长度范围：300-1500像素（基于实际测量）
        # 超长线（>2000）是网格线，应该被惩罚
        if length > 2000:
            # 网格线，严重惩罚
            length_score = 0.0
            ...
            # 接近网格线长度，中等惩罚
            length_score = 0.3
        elif self.r2_length_range[0] <= length <= self.r2_length_range[1]:
            # R2线长度范围，最优
            # 计算在范围内的归一化分数
            range_center = (self.r2_length_range[0] + self.r2_length_range[1]) / 2
            range_width = self.r2_length_range[1] - self.r2_length_range[0]
            dist_from_center = abs(length - range_center)
            length_score = 1.0 - (dist_from_center / range_width)
        elif length >= 200:
            # 可接受的短线条
            length_score = 0.6
        else:
...
def test_r2_detector_v8():
    """Test R2 detector V8 on 5 images"""
    ...
    detector = WKFR2DetectorV8()
    
    print("=" * 80)
    print("WKF R2 Detector V8 - Grid Line Avoidance")
    print("=" * 80)
    print("Grid Line Avoidance Parameters:")
    print(f"  R2 Length Range: {detector.r2_length_range[0]}-{detector.r2_length_range[1]} pixels")
    print(f"  Grid Line Threshold: >2000 pixels (penalized)")
    ...
    print("  Color: BLACK (brightness=3.4-12.9)")
    print("  Length: 300-1500 pixels (medium length, not grid lines)")
    print("=" * 80)
    ...
    # Save results
    output_file = "/root/.openclaw/workspace/r2_detection_v8_results.json"
    try:
