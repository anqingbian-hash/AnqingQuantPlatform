...
# -*- coding: utf-8 -*-
"""
WKF R2 Line Detector V14 - Optimized for Weekly Line
周线优化版 - 调整权重，解决周线误差问题
"""
...
class WKFR2DetectorV14:
    """R2 (Resistance Line) Detector V14 - Optimized for Weekly Line"""
    
    def __init__(self):
        """Initialize detector with optimized weights"""
        self.regions = {
        ...
        # Optimized weights (for weekly line)
        self.selection_weights = {
            'sr_line_constraint_weight': 0.20,  # SR线约束（保持）
            'edge_density_weight': 0.20,        # 提高 (V13: 0.15 -> V14: 0.20)
            'line_length_weight': 0.20,        # 降低 (V13: 0.25 -> V14: 0.20)
            'grid_penalty_weight': 0.20,       # 提高 (V13: 0.15 -> V14: 0.20)
            'brightness_weight': 0.15,          # 保持
            'position_weight': 0.05            # 保持
        }
        
        self.r2_length_range = (200, 1200)  # 缩小范围
        self.grid_line_threshold = 2000
        self.sr_line_y = 122
        self.sr_line_tolerance = 20
    
    def detect_r2_line(self, image_path: str) -> Dict:
        """Detect R2 (resistance line) using optimized weights"""
        print(f"\n[R2 Detection V14] Analyzing: {os.path.basename(image_path)}")
        ...
        # [3] Analyze all horizontal lines with optimized weights
        print("  [3] Analyzing horizontal lines with optimized weights...")
        ...
        for line in lines:
            ...
            score = self._calculate_optimized_score(
        ...
        # [4] Select R2 candidate (highest optimized score)
        print("  [4] Selecting R2 candidate (highest optimized score)...")
        
        if horizontal_lines:
            # Sort by optimized score (highest first)
            ...
            print(f"    Result: R2 detected (highest optimized score)")
    ...
    def _calculate_edge_density(self, edges: np.ndarray, y_center: int, line_length: int, main_chart_width: int) -> float:
        """Calculate edge density for line"""
    ...
    def _calculate_optimized_score(self, length: float, brightness: float, y: int, 
        ...
        # [3] Line length (decreased from 0.25 to 0.20)
        if length < 200:
            length_score = 0.4  # V13: 0.5, V14: 0.4 (decreased)
        elif length < 300:
            length_score = 0.6  # V13: 0.7, V14: 0.6 (decreased)
        elif length < 500:
            length_score = 0.8  # V13: 0.9, V14: 0.8 (decreased)
        elif length < 800:
            length_score = 1.0  # V13: 1.0, V14: 1.0 (unchanged)
        elif length < 1200:
            length_score = 0.6
        else:
            length_score = 0.3  # Grid line, penalized heavily
        score += length_score * self.selection_weights['line_length_weight']
        
        # [4] Grid line penalty (increased from 0.15 to 0.20)
        if has_grid_line:
            grid_penalty_score = 0.4  # V13: 0.6, V14: 0.4 (increased penalty)
        else:
...
def test_r2_detector_v14():
    """Test R2 detector V14 on 5 images"""
    ...
    detector = WKFR2DetectorV14()
    
    print("=" * 80)
    print("WKF R2 Detector V14 - Optimized for Weekly Line")
    print("=" * 80)
    print("Optimized Weights:")
    print(f"  SR Line Constraint Weight: {detector.selection_weights['sr_line_constraint_weight']} (unchanged)")
    print(f"  Edge Density Weight: {detector.selection_weights['edge_density_weight']} (V13: 0.15 -> V14: 0.20)")
    print(f"  Line Length Weight: {detector.selection_weights['line_length_weight']} (V13: 0.25 -> V14: 0.20)")
    print(f"  Grid Penalty Weight: {detector.selection_weights['grid_penalty_weight']} (V13: 0.15 -> V14: 0.20)")
    ...
    print("\nV14 Key Improvements:")
    print("  1. Increased edge_density weight (0.15 -> 0.20)")
    print("  2. Decreased line_length weight (0.25 -> 0.20)")
    print("  3. Increased grid_penalty (0.6 -> 0.4 for grid lines)")
    print("  4. Reduced short line penalty (0.5 -> 0.4, 0.7 -> 0.6)")
    print("\nTarget: Fix weekly line error (1.52, 7.9%)")
    print("=" * 80)
    ...
    # Print summary
    print("\n" + "=" * 80)
    print("R2 Detection V14 Summary")
    print("=" * 80)
        ...
        for result in all_results:
            if result['detected']:
                ...
                if manual_r2_price and price_mapping:
                    ...
                    print(f"  {period}: error={error:.2f} ({error_pct:.1f}%), {grid_status}, {sr_status}, edge_density={edge_density:.3f}")
    ...
    # Save results
    output_file = "/root/.openclaw/workspace/r2_detection_v14_results.json"
    try:
