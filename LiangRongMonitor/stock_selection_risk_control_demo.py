...
class StockSelectionRiskControl:
    ...
    def generate_mock_data(self, stock_list):
        """生成模拟两融数据（演示用）"""
        logger.info("\n【生成模拟数据】")
        ...
        market_df = pd.DataFrame({
            'trade_date': ['20260227', '20260228', '20260302'],
            'exchange_id': ['SSE', 'SZSE', 'BSE'],
            'rzye': [1353687049267.0, 1343687049267.0, 1343687049267.0],
            'rzmre': [135308934416.0, 135308934416.0, 143343060065.0],
            'rzche': [132471642997.0, 132471642997.0, 141568816084.0],
            'rqye': [12028518779.0, 12028518779.0, 5693549294.0],
            'rqmcl': [0.0, 0.0, 28347313.0],
            'rzrqye': [1355715568046.0, 1355715568046.0, 1309839721842.0],
            'rqyl': [22.0, 0.0, 11464.0]
        })
        ...
        for stock_code in stock_list:
            ...
            # 模拟融资余额（不同股票有不同趋势）
            base_margin = np.random.uniform(1e8, 5e8)  # 1-5亿元
            trend = np.random.choice([-0.02, -0.01, 0.0, 0.01, 0.02, 0.05, 0.1, 0.2])  # 随机趋势
            
            rzye_values = []
            for i, date in enumerate(dates):
                growth = trend * i
                noise = np.random.uniform(-0.02, 0.02)  # 随机波动
                margin = base_margin * (1 + growth + noise)
                rzye_values.append(margin)
            
            # 创建DataFrame
            df = pd.DataFrame({
                'trade_date': dates,
                '标的证券代码': [stock_code.split('.')[0]] * 10,
                'rzye': rzye_values,
                'rqye': [rz * 0.01 for rz in rzye_values]  # 融券余额约为1%
            })
            ...
            logger.info(f"  ✓ {stock_code}: 模拟10天数据，变化率: {df['融资余额变化率'].iloc[-1]:.2f}%")
    ...
    def run_risk_control_analysis(self, stock_list):
        ...
        try:
            ...
            stocks_data = self.generate_mock_data(stock_list)
            ...
            market_df = stocks_data['market']['df']
