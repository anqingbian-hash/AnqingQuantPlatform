#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""OpenClaw 统一量化交易平台 v8.0 - Tushare真实数据版"""
import os
import sys
import json
import datetime
...
<head>
    ...
    <title>OpenClaw 统一量化交易平台 v8.0</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .nav li a { color: #666; text-decoration: none; font-size: 16px; padding: 10px 20px; border-radius: 5px; transition: all 0.3s; }
        ...
        function analyzeChip() {
            ...
            fetch('/api/realtime/' + symbol)
                ...
                .then(data => {
                    ...
                    if (data.success) {
                        const stock = data.data;
                        let html = '<div class="data-header"><h3>' + stock.name + ' (' + stock.symbol + ') - 筹码分布</h3><div class="data-source">数据源: ' + data.source.toUpperCase() + '<span class="real-badge">真实</span></div></div><div class="stock-info"><div class="info-item"><label>集中度</label><span>中度集中</span></div><div class="info-item"><label>成本区域</label><span>' + (stock.price * 0.95).toFixed(2) + ' - ' + (stock.price * 1.05).toFixed(2) + '</span></div></div><div style="margin-top: 20px;"><h4 style="color: #333; margin-bottom: 10px;">筹码分布图</h4><div style="background: #f5f5f5; padding: 15px; border-radius: 5px;"><div style="display: flex; align-items: center; margin-bottom: 10px;"><div style="width: 80px; font-size: 12px; color: #666;">' + (stock.price * 1.07).toFixed(2) + '</div><div style="flex: 1; height: 20px; background: #ffc107; border-radius: 3px; margin-left: 10px; width: 10%;"></div><div style="margin-left: 10px; font-size: 12px; color: #666;">10%</div></div></div><div style="display: flex; align-items: center; margin-bottom: 10px;"><div style="width: 80px; font-size: 12px; color: #666;">' + (stock.price * 1.03).toFixed(2) + '</div><div style="flex: 1; height: 20px; background: #ff9800; border-radius: 3px; margin-left: 10px; width: 20%;"></div><div style="margin-left: 10px; font-size: 12px; color: #666;">20%</div></div></div><div style="display: flex; align-items: center; margin-bottom: 10px;"><div style="width: 80px; font-size: 12px; color: #666;">' + stock.price.toFixed(2) + '</div><div style="flex: 1; height: � 20px; background: #00c853; border-radius: 3px; margin-left: 10px; width: 35%;"></div><div style="margin-left: 10px; font-size: 12px; color: #666;">35%</div></div></div><div style="display: flex; align-items: center; margin-bottom: 10px;"><div style="width: 80px; font-size: 12px; color: #666;">' + (stock.price * 0.97).toFixed(2) + '</div><div style="flex: 1; height: 20px; background: #2196f3; border-radius: 3px; margin-left: 10px; width: 25%;"></div><div style="margin-left: 10px; font-size: 12px; color: #666;">25%</div></div></div><div style="display: flex; align-items: center; margin-bottom: 10px;"><div style="width: 80px; font-size: 12px; color: #666;">' + (stock.price * 0.93).toFixed(2) + '</div><div style="flex: 1; height: 20px; background: #9e9e9e; border-radius: 3px; margin-left: 10px; width: 10%;"></div><div style="margin-left: 10px; font-size: 12px; color: #666;">10%</div></div></div><div style="display: flex; align-items: center;"><div style="width: 80px; font-size: 12px; color: #666;">' + (stock.price * 0.87).toFixed(2) + '</div><div style="flex: 1; height: 20px; background: #ffc107; border-radius: 3px; margin-left: 10px; width: 10%;"></div><div style="margin-left: 10px; font-size: 12px; color: #666;">10%</div></div></div><div style="display: flex; align-items: center;"><div style="width: 80px; font-size: 12px; color: #666;">' + (stock.price * 0.83).toFixed(2) + '</div><div style="flex: 1; height: 20px; background: #ff9800; border-radius: 3px; margin-left: 10px; width: 20%;"></div><div style="margin-left: 10px; font-size: 12px; color: #666;">20%</div></div></div><div style="display: flex; align-items: center;"><div style="width: 80px; font-size: 12px; color: #666;">' + (stock.price * 0.79).toFixed(2) + '</div><div style="flex: 1; height: 20px; background: #2196f3; border-radius: 3px; margin-left: 10px; width: 25%;"></div><div style="margin-left: 10px; font-size: 12px; color: #666;">25%</div></div></div><div style="display: flex; align-items: center;"><div style="width: 80px; font-size: 12px; color: #666;">' + (stock.price * 0.75).toFixed(2) + '</div><div style="flex: 1; height: 20px; background: #9e9e9e; border-radius: 3px; margin-left: 10px; width: 35%;"></div><div style="margin-left: 10px; font-size: 12px; color: #666;">35%</div></div></div></div><div style="display: flex; align-items: center;"><div style="width: 80px; font-size: 12px; color: #666;">' + (stock.price * 0.71).toFixed(2) + '</div><div style="flex: 1; height: 20px; background: #ffc107; border-radius: 3px; margin-left: 10px; width: 10%;"></div><div style="margin-left: 10px; font-size: 12px; color: #666;">10%</div></div></div>';
        ...
        function updateDataStatus() {
            fetch('/api/status')
                ...
                .then(data => {
                    ...
                    if (data.data_loaded) {
                        statusText.innerHTML = '✅ 数据已加载: ' + data.stock_count + ' 只股票<br>最后更新: ' + (data.last_update ? new Date(data.last_update).toLocaleString() : '未知') + '<br>数据源: ' + data.source;
                    } else {
...
def status():
    return jsonify({
        ...
        'source': data_manager.active_source
    })
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=7000, debug=False)'0.0.0.0', port=7000, debug=False)
