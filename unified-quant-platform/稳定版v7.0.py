...
# -*- coding: utf-8 -*-
"""
OpenClaw 统一量化交易平台 v7.0 - 稳定版（模拟数据 + 完整登录）
"""
...
def get_mock_realtime_data(code):
    """获取模拟实时数据"""
    ...
    name = stock_names.get(code_clean, f'股票{code_clean}')

    # 模拟价格数据
    base_price = float(code_clean) / 100000 if len(code_clean) == 6 else 10.0
    if base_price < 1:
        base_price = 10.0
...
def get_mock_market_data():
    ...
    up_stocks = int(5000 * up_ratio)
    down_stocks = 5000 - up_stocks
            ...
            let html = `
                <div class="data-header">
                    ...
                    <div class="data-source">
                        ...
                        ${data.source === 'demo' ? '<span class="demo-badge">演示</span>' : ''}
                        <br>
                        更新时间: ${data.timestamp}
                    </div>
                </div>
