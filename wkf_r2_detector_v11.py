...
# -*- coding: utf-8 -*-
"""
WKF R2 Line Detector V11 - Edge Density Prioritized
边缘密度优先版 - 大幅提高edge_density权重（0.15 -> 0.25）
"""
...
class WKFR2DetectorV11:
    """R2 (Resistance Line) Detector V11 - Edge Density Prioritized"""
    
    def __init__(self):
        """Initialize detector with edge density prioritization"""
        self.regions = {
        ...
        # Edge density prioritized weights
        self.selection_weights = {
            'edge_density_weight': 0.25,     # 大幅提高（V10: 0.15 -> V11: 0.25）
            'line_length_weight': 0.30,      # 稍微降低（V10: 0.35 -> V11: 0.30）
            'grid_penalty_weight': 0.20,     # 保持（V10: 0.2）
            'brightness_weight': 0.15,       # 保持（V10: 0.15）
            'position_weight': 0.05,         # 降低（V10: 0.1 -> V11: 0.05）
            'color_weight': 0.05            # 保持（V10: 0.05）
        }
        
        self.r2_length_range = (200, 1500)  # 缩小范围，偏向中等长度
        self.grid_line_threshold = 2000
    
    def detect_r2_line(self, image_path: str) -> Dict:
        """Detect R2 (resistance line) using edge density prioritization"""
        print(f"\n[R2 Detection V11] Analyzing: {os.path.basename(image_path)}")
        ...
        # [3] Analyze all horizontal lines with edge density prioritization
        print("  [3] Analyzing horizontal lines with edge density prioritization...")
        ...
        for line in lines:
            ...
            # Calculate edge density prioritized score
            score = self._calculate_edge_density_prioritized_score(
        ...
        # [4] Select R2 candidate (highest edge density prioritized score)
        print("  [4] Selecting R2 candidate (highest edge density prioritized score)...")
        
        if horizontal_lines:
            # Sort by edge density prioritized score (highest first)
            ...
            print(f"    Result: R2 detected (highest edge density prioritized score)")
    ...
    def _calculate_edge_density_prioritized_score(self, length: float, brightness: float, y: int, 
                                                  chart_height: int, color_category: str,
                                                  has_grid_line: bool, nearby_max_length: int,
                                                  edge_density: float) -> float:
        """Calculate edge density prioritized score for R2 candidate"""
        
        score = 0.0
        
        # [1] Edge density (increased from 0.15 to 0.25)
        # 更高的edge_density权重，优先选择边缘密集的位置
        ...
        # [2] Line length (reduced from 0.35 to 0.30)
        if length < 200:
            length_score = 0.2
        elif length < 300:
            length_score = 0.5
        ...
        # [5] Position score (reduced from 0.1 to 0.05)
...
def test_r2_detector_v11():
    """Test R2 detector V11 on 5 images"""
    ...
    detector = WKFR2DetectorV11()
    
    print("=" * 80)
    print("WKF R2 Detector V11 - Edge Density Prioritized")
    print("=" * 80)
    print("Edge Density Prioritized Weights:")
    print(f"  Edge Density Weight: {detector.selection_weights['edge_density_weight']} (V10: 0.15 -> V11: 0.25)")
    print(f"  Line Length Weight: {detector.selection_weights['line_length_weight']} (V10: 0.35 -> V11: 0.30)")
    ...
    print(f"  Position Weight: {detector.selection_weights['position_weight']} (V10: 0.1 -> V11: 0.05)")
    ...
    print("  Edge Density: >= 0.10 (high density is KEY feature)")
    print("  Length: 200-1500px (medium length)")
    print("=" * 80)
    ...
    # Print summary
    print("\n" + "=" * 80)
    print("R2 Detection V11 Summary")
    print("=" * 80)
    ...
    # Save results
    output_file = "/root/.openclaw/workspace/r2_detection_v11_results.json"
    try:
