...
# -*- coding: utf-8 -*-
"""
WKF R2 Line Detector V5 - Color Agnostic
颜色无关版R2（阻力线）识别器 - 基于最长的水平线
"""
...
class WKFR2DetectorV5:
    """R2 (Resistance Line) Detector V5 - Color Agnostic"""
    
    def __init__(self):
        """Initialize detector with color agnostic approach"""
        self.regions = {
            'main_chart': (0, int(1377 * 0.45))
        }
        
        # Color agnostic Hough parameters
        self.hough_params = {
            'rho': 1,
            'theta': np.pi/180,
            'threshold': 15,
    ...
    def detect_r2_line(self, image_path: str) -> Dict:
        """Detect R2 (resistance line) using color agnostic approach"""
        print(f"\n[R2 Detection V5] Analyzing: {os.path.basename(image_path)}")
        ...
        # [3] Analyze all horizontal lines (color agnostic)
        print("  [3] Analyzing all horizontal lines (color agnostic)...")
        ...
        for line in lines:
            ...
            # Analyze color (for information only)
        ...
        # [4] Select R2 candidate (longest horizontal line)
        print("  [4] Selecting R2 candidate (longest horizontal line)...")
        ...
        if horizontal_lines:
            ...
            top_candidates = horizontal_lines[:3]
            
            r2_line = top_candidates[0]
            detected = True
            
            print(f"    Result: R2 detected (longest horizontal line)")
            print(f"      Top 3 candidates:")
            for i, candidate in enumerate(top_candidates, 1):
                print(f"        {i}. y={candidate['y']}, length={candidate['length']}, color={candidate['color_category']}, brightness={candidate['brightness']:.0f}")
        else:
            r2_line = None
            detected = False
...
def test_r2_detector_v5():
    """Test R2 detector V5 on 5 images"""
    ...
    detector = WKFR2DetectorV5()
    
    print("=" * 80)
    print("WKF R2 Detector V5 - Color Agnostic Test")
    print("=" * 80)
    print("Color Agnostic Parameters:")
    print(f"  Hough Threshold: 15")
    print(f"  MinLineLength: 100")
    print(f"  MaxLineGap: 100")
    print(f"  Horizontal tolerance: 10px")
    print(f"  R2 Selection: Longest horizontal line (color agnostic)")
    print("=" * 80)
    ...
    for result in all_results:
        ...
        if detected:
            ...
            print(f"  {period}: ✅ y={r2_y}, {r2_color}, length={r2_length} (valid_lines={valid_lines})")
        else:
            print(f"  {period}: ❌ (valid_lines={valid_lines})")
    
    # Save results
    output_file = "/root/.openclaw/workspace/r2_detection_v5_results.json"
    try:
