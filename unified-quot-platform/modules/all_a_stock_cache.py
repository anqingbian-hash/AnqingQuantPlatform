...
class AllAStockQueryWithCache:
    """全A股查询 - 支持本地缓存"""

    CACHE_DIR = '/root/.openclaw/workspace/unified-quot-platform/data'
    CACHE_FILE = os.path.join(CACHE_DIR, 'all_a_stocks.csv')
    ...
    def _load_cache(self):
        ...
        try:
            ...
            if os.path.exists(self.METADATA_FILE):
                ...
                logger.info(f"[AllAStockQuery] 缓存元数据加载成功")

            # 检查缓存是否过期
            if self._is_cache_valid():
                # 加载股票数据
                if os.path.exists(self.CACHE_FILE):
                    self.all_stock_df = pd.read_csv(self.CACHE_FILE)
                    logger.info(f"[AllAStockQuery] 缓存数据加载成功: {len(self.all_stock_df)} 只股票")
                    return
                else:
    ...
    def _try_update_cache(self, retry: int = 2):
        ...
        for attempt in range(retry):
            try:
                ...
                logger.info(f"[AllAStockQuery] 缓存更新成功: {len(df)} 只股票")
                return True
    ...
    def query_stock(self, code: str, force_refresh: bool = False) -> Dict:
        ...
        # 强制刷新或首次查询，尝试更新缓存
        if force_refresh or self.all_stock_df is None:
    ...
    def _query_from_data(self, code: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"[AllAStockQuery] 查询失败: {e}")
    ...
    def search_stock(self, keyword: str) -> List[Dict]:
        ...
        try:
            results = self.all_stock_df[
                self.all_stock_df['代码'].str.contains(keyword) |
                self.all_stock_df['名称'].str.contains(keyword)
            ].head(20)
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            'cache_age': cache_age,
            'cache_time': self.cache_metadata.get('last_update') if self.cache_metadata else None,
            'status': 'loaded' if self.all_stock_df is not None else 'not_loaded'
        }


def test_all_stock_with_cache():
    """测试全A股查询（带缓存）"""
    print("=" * 70)
    print("全A股查询测试（支持本地缓存）")
    print("=" * 70)

    # 初始化
    query = AllAStockQueryWithCache(cache_enabled=True, cache_expiry=3600)
